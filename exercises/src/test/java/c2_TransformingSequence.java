...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    public void cast() {
        Flux<String> numbersFlux = object_service()
                .flatMapSequential(object -> Flux.just(object.toString()));
    ...
    public void sequence_sum() {
        ...
        Mono<Integer> sum = numerical_service().reduce((a, b) -> {
            int result = a + b;
            System.out.format(
                    "[%d + %d] = %d\n", a, b, result
            );
    ...
    public void sum_each_successive() {
        Flux<Integer> sumEach = numerical_service()
                .scan((acc, value) -> acc + value); // Use scan operator to emit each intermediary number;
    ...
    public void sequence_starts_with_zero() {
        Flux<Integer> result = numerical_service()
                .flatMapSequential(object -> Flux.just(object))
                .startWith(0);
